...
import {
  useCreateDmnDefinition,
  useDmnDefinition,
  useUpdateDmnDefinition,
...
export function DmnEditorPage() {
  ...
  const { data: existing, isLoading } = useDmnDefinition(id ?? null);
  const createDmn = useCreateDmnDefinition();
  const updateDmn = useUpdateDmnDefinition(id ?? "");
  ...
  async function handleSave() {
    ...
    try {
      if (isNew) {
        await createDmn.mutateAsync({ name, description, xml_content: xml });
      } else {
        await updateDmn.mutateAsync({ name, description, xml_content: xml });
      }
      navigate("/workflows/dmn");
    } catch (e) {
      setError(String(e));
    } finally {
      setSaving(false);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", gap: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16, flexShrink: 0 }}>
        ...
        <h2 style={{ margin: 0 }}>{isNew ? "New Decision Table" : `Edit: ${name}`}</h2>
      </div>

      <div style={{ display: "flex", gap: 16, flexShrink: 0 }}>
        <div style={{ flex: 1 }}>
          <label style={labelStyle}>Name *</label>
          <input value={name} onChange={(e) => setName(e.target.value)} style={inputStyle} placeholder="e.g. Approval Threshold" />
        </div>
        <div style={{ flex: 2 }}>
      ...
      <div style={{ flex: 1, minHeight: 0 }}>
        <DmnEditor ref={editorRef} xml={existing?.xml_content} onChange={() => {}} />
      </div>
      ...
      <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
        <button onClick={handleSave} disabled={saving} style={saveBtn}>
          {saving ? "Saving…" : isNew ? "Create Decision Table" : "Save Changes"}
        </button>
